...
export function isPhantomInsider(user: User | null | undefined): boolean {
  ...
  const insiderFlags: unknown[] = [
    appMetadata.insider,
    userMetadata.insider,
  ]

  for (const rawFlag of insiderFlags) {
  ...
  if (appRole === 'insider' || userRole === 'insider') return true
  if (appRole === 'admin' || userRole === 'admin') return true
  if (appRole === 'editor' || userRole === 'editor') return true
