	...
	if (activeProducts.length === 0) {
		await bot.sendMessage(
			...
			formatTerminalTelegramMessage({
				...
				lines: ["No active catalog entities.", "Retry /merch after uplink refresh."],
			}),
			{ parse_mode: "HTML" },
		);
		return;
...
async function createTelegramCheckout(input: {
	...
	const anonymizedUserHash = hashTelegramId(String(input.telegramUserId));
...
async function handleMessage(update: TelegramUpdate) {
	...
	if (command !== "/start" && command !== "/merch") return;
	...
	if (command === "/start") {
		await bot.sendMessage(
			...
			formatTerminalTelegramMessage({
				...
				lines: ["Merch channel linked.", "Command accepted: /merch"],
			}),
			{ parse_mode: "HTML" },
		);
	}
...
export async function POST(req: NextRequest) {
	...
	if (!hasHashSalt) {
		return NextResponse.json({ ok: true, skipped: "missing_hash_salt" });
	}
