...
export default async function ReaderPage() {
  // 1) Cookie-alapú "klubtagság" ellenőrzés
  const cookieStore = await cookies();
  const session = cookieStore.get("reader_session");

  if (!session || session.value !== "granted") {
    redirect(`/reader-access?redirect=${encodeURIComponent("/reader")}`);
  }

  // 2) Novellák betöltése
