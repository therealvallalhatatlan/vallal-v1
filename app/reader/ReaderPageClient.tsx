...
export default function ReaderPageClient({
  ...
  useEffect(() => {
    ...
    if (!session) {
      router.replace(`/auth?from=${encodeURIComponent(requestedPath)}`);
      return;
    }
    ...
    const fetchStories = async () => {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403 && body?.error === "no_access") {
          router.replace(`/reader/no-access?from=${encodeURIComponent(requestedPath)}`);
          return;
        }
    ...
    fetchStories();
  }, [loading, requestedPath, router, session]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="rounded-xl border border-neutral-800 bg-black/70 px-6 py-4 text-sm text-neutral-200">
          ...
          <p>{error}</p>
          <button
            onClick={() => router.replace(`/auth?from=${encodeURIComponent(requestedPath)}`)}
