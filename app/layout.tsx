...
export const metadata: Metadata = {
  ...
  openGraph: {
    type: "book", url: "/", siteName: "Vállalhatatlan", title: "Vállalhatatlan — Meg sem történt történetek",
    description: "Nyers, sötét humorú novellák a 90-es évek/ezredforduló Budapestjéről: techno, acid, rave, drogkultúra és töréspontok.",
    images: [{ url: "/og.png", width: 1200, height: 630, alt: "Vállalhatatlan — Meg sem történt történetek" }], locale: "hu_HU", alternateLocale: ["en_US"],
  },
...
║  THERE ARE MORE LAYERS.  THERE ARE MORE NODES.  THERE ARE MORE POINTS.       ║
║  > OPEN THE CONSOLE                                                          ║
║  > TYPE: help()                                                              ║
║  YOUR OBJECTIVE: FIND AS MANY POINTS AS YOU CAN.                            ║
║  MAXIMUM SCORE = MAXIMUM ACCESS.                                            ║
║  NODE STATUS: ONLINE   TRACE STATUS: UNKNOWN   CHANNEL: INTERNAL             ║
║  OBSERVER: DETECTED    SCORE: 0                                             ║
║  THERE IS NOTHING TO STEAL. THERE IS ONLY SOMETHING TO FIND.                 ║
╚══════════════════════════════════════════════════════════════════════════════╝
-->` }} />
  ...
  var MAX_SCORE = 300;
  var SCORE_KEY = "vh_curiosity_score_v1";
  var DISCOVERY_KEY = "vh_curiosity_discoveries_v1";

  function readScore() {
    try {
      var value = Number(window.localStorage.getItem(SCORE_KEY) || "0");
  ...
  function readDiscoveries() {
    try {
      var value = JSON.parse(window.localStorage.getItem(DISCOVERY_KEY) || "[]");
  ...
  function stubNotice(command) {
    console.log("%cBOOTSTRAP MODE%c " + command + " is available. The full curiosity engine is attaching...", "color:#d9f99d;font-weight:800", "color:#a3a3a3;font-family:monospace");
  }

  window.help = window.help || function () {
  ...
  window.score = window.score || function () {
    var current = readScore();
    print("SCOREBOARD", [
      "CURRENT SCORE: " + current,
      "MAXIMUM SCORE: " + MAX_SCORE,
      "REMAINING: " + Math.max(0, MAX_SCORE - current),
      "",
      current >= MAX_SCORE ? "MAXIMUM ACCESS ACHIEVED." : "THERE ARE MORE POINTS TO FIND."
    ]);
    return { score: current, max: MAX_SCORE, remaining: Math.max(0, MAX_SCORE - current) };
  };

  window.whoami = window.whoami || function () { print("IDENTITY UNKNOWN", ["SOURCE INSPECTION: POSSIBLE", "OBSERVER: DETECTED"]); stubNotice("whoami()"); return "observer detected"; };
  window.where = window.where || function () { print("NODE LOCATION", ["NETWORK: VALLALHATATLAN", "NODE: 07", "STATUS: STILL RUNNING"]); stubNotice("where()"); return "/NODE-07"; };
  window.remember = window.remember || function () { var items = readDiscoveries(); print("MEMORY", items.length ? items.map(function (item) { return "  " + item; }) : ["  nothing yet", "", "Keep looking."]); stubNotice("remember()"); return items; };
  window.scan = window.scan || function () { print("NODE SCAN", ["PUBLIC SURFACE: ONLINE", "HIDDEN SURFACE: YES", "NODE 07: LISTENING"]); stubNotice("scan()"); return true; };
  window.coffee = window.coffee || function () { console.log("%ccoffee.exe was never installed", "color:#a3a3a3;font-family:monospace"); stubNotice("coffee()"); return "404: caffeine not found"; };
  window.sudo = window.sudo || function () { console.warn("permission denied"); console.log("root is not a role. root is a personality disorder."); stubNotice("sudo()"); return false; };
  window.openUnknown = window.openUnknown || function () { window.location.href = "/unknown"; return "/unknown"; };
