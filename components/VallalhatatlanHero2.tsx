            ...
            .map((spot) => ({
              ...
              remaining_quantity:
                typeof spot.remaining_quantity === "number"
                  ...
                  : null,
              total_quantity:
                typeof spot.total_quantity === "number"
                  ? spot.total_quantity
                  : null,
...
@keyframes modalIn { from { opacity: 0; transform: translateY(50px); } to { opacity: 1; transform: translateY(0); } }`}</style>
    ...
    >
      ...
      <div className="relative z-20 flex min-h-0 flex-1 flex-col overflow-y-auto px-6">
        ...
        <section className="mt-16 w-full" aria-label="Live Network">
          <div
            className="mb-3 flex items-center justify-between border-t border-zinc-800 pt-4 text-[11px] uppercase tracking-[0.24em] text-zinc-200"
            style={{ fontFamily: "var(--font-mono-tech)" }}
          >
            <div>
              <span className="text-lime-100/80">LIVE NETWORK</span>
              <span className="ml-3 text-zinc-700">● SIGNAL FEED</span>
            </div>
            <Link
              href="/halozat"
              className="text-zinc-500 transition-colors hover:text-lime-100"
            >
              HÁLÓZAT ↗
            </Link>
          </div>

          <div className="relative overflow-hidden border border-zinc-800 bg-[#040404]">
            <div className="pointer-events-none absolute inset-0 opacity-40 [background-image:linear-gradient(rgba(255,255,255,0.035)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] [background-size:24px_24px]" />
            <div className="pointer-events-none absolute inset-x-0 top-1/2 h-px bg-zinc-800/70" />
            <div className="pointer-events-none absolute left-1/2 top-0 h-full w-px bg-zinc-800/70" />

            <div className="relative min-h-[230px] overflow-hidden p-4 sm:min-h-[270px] sm:p-6">
              <div className="absolute left-1/2 top-1/2 h-32 w-32 -translate-x-1/2 -translate-y-1/2 rounded-full border border-zinc-800 sm:h-40 sm:w-40" />
              <div className="absolute left-1/2 top-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full border border-zinc-800 sm:h-24 sm:w-24" />
              <div className="absolute left-1/2 top-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-lime-100 shadow-[0_0_18px_rgba(236,252,203,0.8)]" />

              {networkLoading ? (
                <div
                  className="absolute inset-0 flex items-center justify-center text-[10px] uppercase tracking-[0.2em] text-zinc-600"
                  style={{ fontFamily: "var(--font-mono-tech)" }}
                >
                  SIGNAL SCANNING...
                </div>
              ) : networkSpots.length === 0 ? (
                <div
                  className="absolute inset-0 flex items-center justify-center text-center text-[10px] uppercase tracking-[0.18em] text-zinc-600"
                  style={{ fontFamily: "var(--font-mono-tech)" }}
                >
              ...
              <div
                className="absolute bottom-3 left-4 flex gap-4 text-[8px] uppercase tracking-[0.14em] text-zinc-500"
                style={{ fontFamily: "var(--font-mono-tech)" }}
              >
                <span><span className="mr-1 inline-block h-1.5 w-1.5 rounded-full bg-lime-100" />FIZIKAI</span>
                <span><span className="mr-1 inline-block h-1.5 w-1.5 rounded-[2px] bg-fuchsia-200" />DIGITÁLIS</span>
              </div>
            ...
            <div className="grid grid-cols-2 border-t border-zinc-800 sm:grid-cols-4">
              {[
                {
                  label: "ELÉRHETŐ FIZIKAI",
                  value: networkSpots.filter(
                    (spot) =>
                      spot.type === "physical" &&
                      (spot.remaining_quantity === null || spot.remaining_quantity > 0),
                  ).length,
                ...
                {
                  label: "DIGITÁLIS",
                  value: networkSpots.filter((spot) => spot.type === "virtual").length,
                },
                {
                  label: "AKTÍV PONT",
                  value: networkSpots.length,
                },
                {
                  label: "ÁLLAPOT",
                  value: networkLoading ? "SCAN" : networkSpots.length > 0 ? "ONLINE" : "QUIET",
                },
              ].map((stat, index) => (
                <div
                  key={stat.label}
                  className={"px-4 py-4 " + (index > 0 ? "border-l border-zinc-800 " : "") + (index > 1 ? "border-t border-zinc-800 sm:border-t-0" : "")}
                >
                  <p
                    className="text-[8px] uppercase tracking-[0.14em] text-zinc-600"
                    style={{ fontFamily: "var(--font-mono-tech)" }}
                  >
                    {stat.label}
                  </p>
            ...
            <Link
              href="/halozat"
              className="group flex items-center justify-between border-t border-zinc-800 px-4 py-4 text-[10px] uppercase tracking-[0.18em] text-zinc-500 transition-colors hover:bg-zinc-100/[0.03] hover:text-zinc-100 sm:px-6"
              style={{ fontFamily: "var(--font-mono-tech)" }}
            >
              <span>Belépés az aktív hálózatba</span>
              <span className="text-lg text-zinc-700 transition-transform group-hover:translate-x-1 group-hover:text-lime-100">→</span>
            </Link>
          </div>
        </section>
